...
void display()
{
	...
	//glScalef(1, 1, 1);							// 말 그대로 물체의 크기를 크게 키우거나 줄이는 것 -> 물체 자체가 커짐
	
	
	//glTranslatef(1,0,0);
	//glRotatef(1, 1, 1, 1);

	//gluLookAt(0, 0, 3, 0, 0, 0, 0, 1, 0);		// (내가보는위치 -> 인자 3개, 보는 방향 -> 인자 3개, up -> 인자 3개)
												// 보는 위치를 기준으로 rotation 가능
	...
	for (register int j = 0; j < 100000; j++) {
		float color = (vertex[j].Z + 0.7) / 2.2;
